...
- таймфрейм для `Топ-150 по объему`: `5m`, `15m`, `30m`;
- таймфрейм для `Популярные 30` и `Мои пары`: `1m`, `3m`, `5m`, `15m`, `30m`;
...
Быстрые таймфреймы `1m` и `3m` доступны только для сфокусированных списков `Популярные 30` и `Мои пары`. Для `Топ-150 по объему` бот оставляет минимум `5m`, чтобы не разгонять шум и количество запросов. Если переключиться с `1m` обратно на `Топ-150`, таймфрейм автоматически станет `5m`.
