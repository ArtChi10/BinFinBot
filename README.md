...
- фиксированный список популярных 30 Bybit spot `/USDT` пар;
...
Режим `Топ-150 по объему` обновляет список пар раз в час по 24h quote volume Bybit. Режим `Популярные 30` использует фиксированный список крупных USDT-пар: `BTC`, `ETH`, `SOL`, `XRP`, `BNB`, `DOGE`, `ADA`, `AVAX`, `LINK`, `TRX`, `TON`, `SUI`, `LTC`, `BCH`, `DOT`, `UNI`, `AAVE`, `NEAR`, `ETC`, `FIL`, `ATOM`, `ARB`, `OP`, `INJ`, `WLD`, `PEPE`, `SHIB`, `BONK`, `MNT`, `ONDO`. Если отдельная пара временно недоступна на Bybit spot, бот пропустит ее и продолжит мониторинг остальных.
...
- использует фиксированный список, если выбран режим `Популярные 30`;
