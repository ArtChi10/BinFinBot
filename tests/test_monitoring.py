...
from src.monitoring.monitor import _symbols_by_user, closed_ohlcv_candles
...
class MonitoringTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_symbols_by_user_resolves_dynamic_and_selected_popular_lists(
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            await set_all_user_popular_pair_selections(
                database_path,
                telegram_user_id=2,
                selected=False,
            )
            await toggle_user_popular_pair_selection(
                database_path,
                telegram_user_id=2,
                symbol="BTC/USDT",
            )
            ...
            result = await _symbols_by_user(
                database_path,
                [top_user, popular_user],
                top_pairs,
            )

        self.assertEqual(result[1], set(top_pairs))
        self.assertEqual(result[2], {"BTC/USDT"})

    async def test_symbols_by_user_allows_empty_popular_selection(self) -> None:
        with TemporaryDirectory() as temp_dir:
            database_path = str(Path(temp_dir) / "test.sqlite3")
            await init_db(database_path)
            await set_all_user_popular_pair_selections(
    ...
    def test_user_settings_fixture_supports_pair_universe(self) -> None:
        top_user = _settings(telegram_user_id=1, pair_universe=PAIR_UNIVERSE_TOP_150)
        popular_user = _settings(
            telegram_user_id=2,
            pair_universe=PAIR_UNIVERSE_POPULAR_30,
        ...
        self.assertEqual(top_user.pair_universe, PAIR_UNIVERSE_TOP_150)
        self.assertEqual(popular_user.pair_universe, PAIR_UNIVERSE_POPULAR_30)
